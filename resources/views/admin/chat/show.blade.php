<x-dashboard-layout>
        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white rounded-lg shadow-lg overflow-hidden">
                    ...
                    @foreach($messages as $message)
                        <div class="flex {{ $message->sender_id === auth()->id() ? 'justify-end' : 'justify-start' }}">
                            <div class="flex items-end space-x-2 max-w-[70%]">
                                ...
                                @endif
                                <div class="{{ $message->sender_id === auth()->id() ? 'bg-blue-600 text-white' : 'bg-white text-gray-900' }} rounded-2xl px-4 py-2 shadow-sm">
                ...
                <div class="p-4 border-t bg-white">
                    <form id="messageForm" class="flex space-x-2" action="{{ route('admin.chat.send', $receiver->id) }}" method="POST">
                        @csrf
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // Listen to messages sent by the other user (intern)
            Echo.channel(`chat.${recipientId}.${senderId}`)
                .listen('.MessageSent', (e) => {
                    ...
                    div.innerHTML = `
                        <div class="flex items-end space-x-2 max-w-[70%]">
                            <div class="w-8 h-8 bg-blue-600 rounded-full flex items-center justify-center text-white font-bold text-sm flex-shrink-0">
                                ${e.message.sender.name.charAt(0).toUpperCase()}
                            </div>
