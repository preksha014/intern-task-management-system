        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-md rounded-xl p-8">
                <div class="mb-4 text-center">
                    ...
                    <p class="mt-1 text-sm text-gray-600">Update the task information below</p>
                </div>

                <form action="{{ route('tasks.update', $task) }}" id="edit-task-form" method="POST" class="space-y-5">
                    @csrf
                    @method('PATCH')
                    ...
                    <div class="space-y-1">
                        ...
                        <select name="interns[]" id="interns" multiple
                            ...
                            @foreach($interns as $intern)
                                <option value="{{ $intern->id }}" {{ in_array($intern->id, old('interns', $task->interns->pluck('id')->toArray())) ? 'selected' : '' }}>
                                    {{ $intern->user->name }}
                                </option>
                            @endforeach
                        </select>
